...
public class bindarySearch {
    public static void main(String[] args) {
        ...
        System.out.println(bindarySearchLeft(nums2, 6));
        System.out.println(bindarySearchRight(nums2, 0));
    }
    ...
    // 适用场景
        // 存在重复元素时找第一次出现的位置
        // 查找大于等于目标值的第一个元素
    public static int bindarySearchLeft(int[] nums, int target){
    ...
    // 适用场景
        // 存在重复元素时找最后一次出现的位置
        // 查找小于等于目标值的最后一个元素
    public static int bindarySearchRight(int[] nums, int target){
        int left = 0, right = nums.length-1, mid = 0, res = -1;
        while(left <= right){
            mid = left + (right-left)/2;
            if(nums[mid] <= target){
